...
#calculate total likelihood genome list for a node.
#created this function to make creation of this genome list consistent;
#could be further imptoved to 
def getTot(node,mutMatrix):
	if node.up==None:
		newTot=rootVector(node.probVect,False,mutMatrix)
	elif node==node.up.children[0]:
		newTot=mergeVectorsUpDown(node.up.probVectUpRight,node.dist,node.probVect,False,mutMatrix)
	else:
		newTot=mergeVectorsUpDown(node.up.probVectUpLeft,node.dist,node.probVect,False,mutMatrix)
	return newTot
...
def findBestParentInitial(t1,diffs,sample,mutMatrix):
	bestNodeSoFar=t1
	...
	while nodesToVisit:
		...
		if not t1.children: #check if the new leaf is strictly less informative than already placed leaf
			...
			if comparison==1:
				...
				return t1, 1.0, False, float("-inf"), float("-inf"), False
			elif comparison==2:
		...
		if strictInitialStopRulesInitial:
			if failedPasses<=allowedFailsInitial and LKdiff>(bestLKdiff-thresholdLogLKinitial): 
				for c in t1.children:
					nodesToVisit.append((c,LKdiff,failedPasses))
		else:
			if failedPasses<=allowedFailsInitial or LKdiff>(bestLKdiff-thresholdLogLKinitial): 
				for c in t1.children:
					nodesToVisit.append((c,LKdiff,failedPasses))
	return bestNodeSoFar , bestLKdiff , bestIsMidNode, bestUpLK, bestDownLK, adjustBLen
...
def findBestParent(node,child,bestLKdiff,removedBLen,mutMatrix,topologyPhase=True,newSamplePartials=None,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology,isMidNode=False,bestUpLK=float("-inf"),bestDownLK=float("-inf")):
	bestNodeSoFar=node
	bestIsMidNode=isMidNode
	nodesToVisit=[]
	bestUpLK=bestUpLK
	bestDownLK=bestDownLK
	...
	if topologyPhase:
		...
		removedPartials=newSamplePartials
		if bestIsMidNode:
			downLK=bestDownLK
		else:
			downLK=bestLKdiff
		if node.up!=None:
			if node.up.children[0]==node:
				...
				childUp=2
				vectUpUp=node.up.probVectUpLeft
			nodesToVisit.append((node.up,childUp,node.probVect,node.dist,False,downLK,0))
		if node.children: 
			nodesToVisit.append((node.children[0],0,node.probVectUpRight,node.children[0].dist,False,downLK,0))
			nodesToVisit.append((node.children[1],0,node.probVectUpLeft,node.children[1].dist,False,downLK,0))
			...
			if t1.dist:
				if not (t1.up==node or t1.up==None): #try to append mid-branch
					...
					if topologyPhase:
						...
						midProb=appendProb(midTot,removedPartials,removedBLen,mutMatrix)
				...
				if topologyPhase:
					...
					nodeProb=appendProb(nodeTot,removedPartials,removedBLen,mutMatrix)
	...
	if topologyPhase:
		...
		bestChild=None
		if bestIsMidNode:
			nodeUp=bestNodeSoFar.up
			while (not nodeUp.dist) and (nodeUp.up!=None):
				nodeUp=nodeUp.up
			bestUpLK=appendProb(nodeUp.probVectTot,removedPartials,removedBLen,mutMatrix)
			return bestNodeSoFar , bestLKdiff , bestIsMidNode, bestUpLK, bestDownLK, bestNodeSoFar
		else:
			...
			while nodesToVisit:
				...
				if not t1.dist:
					...
					while True:
						newLKdiff2=appendProb(newProbVect2,removedPartials,removedBLen,mutMatrix)
			...
			return bestNodeSoFar , bestLKdiff , bestIsMidNode, bestUpLK, bestOfChildLK, bestChild
...
def findBestParentOld(t1,diffs,sample,mutMatrix):
	bestNodeSoFar=t1
	bestLKdiff=float("-inf")
	bestIsMidNode=False
	bestUpLK=float("-inf")
		...
		if direction==2:
			...
			if not updatedBLen:
				if node.dist or node.up==None:
					...
					if newTot==None and (not node.dist):
						updateBLen(nodeList,node,mutMatrix)
						updatedBLen=True
					elif newTot==None:
...
def placeSampleOnTree(node,newPartials,sample,newChildLK,isMidNode, bestUpLK, bestDownLK, bestDownNode,mutMatrix,pseudoMutCounts, adjustBLen):
	...
	if isMidNode:
		...
		newInternalNode.probVectTot=getTot(newInternalNode,mutMatrix)
		...
		if bestLen:
			newNode.probVectTot=getTot(newNode,mutMatrix)
...
def estimateBranchLengthWithDerivative(probVectP,probVectC,mutMatrix):
	c1=0.0
	ais=[]
	indexEntry1, indexEntry2, pos = 0, 0, 0
	entry1=probVectP[indexEntry1]
...
def traverseTreeToOptimizeBranchLengths(root,mutMatrix,testing=False):
...
# bestLKdiff is the best likelihood cost found for the current placement (after optimizing the branch length).
# removedBLen is such branch length that optimizes the current placement - it will be used to place the subtree attached at other nodes of the tree.
def findBestParentTopologyOld(node,child,bestLKdiff,removedBLen,mutMatrix):
	bestNodeSoFar=node
	bestIsMidNode=False
	nodesToVisit=[]
	removedPartials=node.children[child].probVect
...
def placeSubtreeOnTree(node,newPartials,appendedNode,newBranchL,newChildLK,isMidNode,mutMatrix):
	...
	if isMidNode and node.up!=None: #best appending location found so far was mid-branch above node
		...
		newVect=getTot(newInternalNode,mutMatrix)
...
def traverseTreeForTopologyUpdate(node,mutMatrix,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology,thresholdTopologyPlacement=thresholdTopologyPlacement):
	#verbose=True
...
def startTopologyUpdates(node,mutMatrix,checkEachSPR=False,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology,thresholdTopologyPlacement=thresholdTopologyPlacement):
	...
	while nodesToVisit:
		...
		if newNode.dirty:
			...
			newRoot2,improvement=traverseTreeForTopologyUpdate(newNode,mutMatrix,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology,thresholdTopologyPlacement=thresholdTopologyPlacement)
			if checkEachSPR:
					...
					if entry2[0]==5: # case N
						...
						if entry1[0]<5:
							if len(entry1)==2:
								...
								#therefore commented out bit below
								#if len(entry1)==4:
								#	totLen1+=entry1[3]
						else:
							if len(entry1)==3:
							...
							if entry2[0]==4:
									...
									if len(entry1)==4:
											...
											if i1==i:
												...
												for j in range4:
													if j==i:
														...
														tot3=prob*mutMatrix[i][j]*contribLength*entry2[-1][j]
														waitingTimes[i]+=(contribLength/2)*tot3/normalization
														waitingTimes[j]+=(contribLength/2)*tot3/normalization
														counts[i][j]+=tot3/normalization
											else:
												prob=rootFreqs[i]*mutMatrix[i][i1]*entry1[2]*(1.0+nonMutRates[i]*contribLength)*entry2[-1][i]
												waitingTimes[i]+=contribLength*prob/normalization
												#waitingTimes[i1]+=(entry1[2]/2)*prob/normalization
												#counts[i][i1]+=prob/normalization
									else:
										for i in range4:
											if i1==i:
										...
										for i in range4:
											if i1==i:
												...
												prob=mutMatrix[i1][i]*totLen1*entry2[-1][i]
												waitingTimes[i1]+=(totLen1/2)*prob/normalization
												waitingTimes[i]+=(totLen1/2)*prob/normalization
												counts[i1][i]+=prob/normalization
								pos+=1
								...
								if not totLen2:
									...
									if len(entry1)<4:
										...
										waitingTimes[i1]+=(contribLength/2)*prob1/normalization
										waitingTimes[i2]+=(contribLength/2)*prob1/normalization
										counts[i1][i2]+=prob1/normalization
										waitingTimes[i2]+=contribLength*prob2/normalization
										#waitingTimes[i1]+=(entry1[2]/2)*prob2/normalization
										#counts[i2][i1]+=prob2/normalization
								pos+=1
								...
								if entry2[0]==6:
									...
									for i in range4:
										for j in range4:
											if i==j:
												...
												prob=entry1[-1][i]*mutMatrix[i][j]*totLen1*entry2[-1][j]
												waitingTimes[i]+=(totLen1/2)*prob/normalization
												waitingTimes[j]+=(totLen1/2)*prob/normalization
												counts[i][j]+=prob/normalization
								else:
									if entry2[0]==4:
										i2=refIndeces[pos]
									else:
									...
									for i in range4:
										if i==i2:
											...
											prob=entry1[-1][i]*mutMatrix[i][i2]*totLen1
											waitingTimes[i]+=(totLen1/2)*prob/normalization
											waitingTimes[i2]+=(totLen1/2)*prob/normalization
											counts[i][i2]+=prob/normalization
							pos+=1
...
while distances:
	...
	if fastPreliminaryPlacement:
		node , bestNewLK , bestIsMidNode, bestUpLK, bestDownLK, adjustBLen = findBestParentInitial(t1,newPartials,sample,mutMatrix)
		comparing=False
		if comparing:
			initialValues=(node , bestNewLK , bestIsMidNode, bestUpLK, bestDownLK, adjustBLen)
		if bestNewLK<0.5:
			if adjustBLen:
				bLen=oneMutBLen*bLenAdjustment
			else:
				bLen=oneMutBLen
			node , bestNewLK , isMidNode, bestUpLK, bestDownLK, bestDownNode = findBestParent(node,None,bestNewLK,bLen,mutMatrix,topologyPhase=False,newSamplePartials=newPartials,isMidNode=bestIsMidNode,bestUpLK=bestUpLK,bestDownLK=bestDownLK)
			if comparing:
				improvedValues=(node , bestNewLK , isMidNode, bestUpLK, bestDownLK, bestDownNode)
			if comparing:
				node , bestNewLK, isMidNode, bestUpLK, bestDownLK, bestDownNode, adjustBLen=findBestParentOld(t1,newPartials,sample,mutMatrix)
				if abs(bestNewLK-improvedValues[1])>0.01:
					print("Initial values")
					print(initialValues)
					print("Improved values")
					print(improvedValues)
					print("traditional placement")
					print(node , bestNewLK, isMidNode, bestUpLK, bestDownLK, bestDownNode, adjustBLen)
...
if inputTree=="" or largeUpdate:
	...
	if model!="JC":
		mutMatrix = expectationMaximizationCalculationRates(t1)
		for i in range4:
			nonMutRates[i]=mutMatrix[i][i]
		for i in range(lRef):
			cumulativeRate[i+1]=cumulativeRate[i]+nonMutRates[refIndeces[i]]
		reCalculateAllGenomeLists(t1,mutMatrix,countNodes=False)
...
if fastTopologyInitialSearch and (inputTree=="" or largeUpdate):
	...
	#if inputTree=="" or largeUpdate:
	setAllDirty(t1)
	newRoot,improvement=startTopologyUpdates(t1,mutMatrix,checkEachSPR=debugging,strictTopologyStopRules=strictTopologyStopRulesInitial,allowedFailsTopology=allowedFailsTopologyInitial,thresholdLogLKtopology=thresholdLogLKtopologyInitial,thresholdTopologyPlacement=thresholdTopologyPlacementInitial)
	if newRoot!=None:
		t1=newRoot
...
if optimizeBranchLengths:
	...
	while subRound<20:
		if improvement<thresholdLogLKwholeTopologyImprovement:
			break
		subRound+=1
		improvement=traverseTreeToOptimizeBranchLengths(t1,mutMatrix)
		if testingBLen:
