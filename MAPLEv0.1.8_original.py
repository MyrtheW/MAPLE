...
parser = argparse.ArgumentParser(
	description='Estimate a tree from a diff format and using iterative approximate maximum likelihood sample placement.')
parser.add_argument('--input',
                    default="/Users/demaio/Desktop/GISAID-hCoV-19-phylogeny-2021-03-12/phylogenetic_inference/2021-03-31_unmasked_differences_reduced.txt_consensus-based.txt",
                    help='input MAPLE file name; should contain first the reference genome and then the difference of all samples with respet to the reference.')
parser.add_argument('--reference', default="",
                    help='optional input reference file name. By default it assumes instead that the reference is part of the MAPLE format input.')
parser.add_argument('--output',
                    default="/Users/demaio/Desktop/GISAID-hCoV-19-phylogeny-2021-03-12/phylogenetic_inference/MAPLE",
                    help='output path and identifier to be used for newick output file.')
parser.add_argument('--inputTree', default="",
                    help='input newick tree file name; this is optional, and is used for online inference (or for Robinson-Foulds distance calculation if option --inputRFtrees is also used).')
parser.add_argument("--largeUpdate",
                    help="When using option --inputTree to do online inference, by default the tree is only updated locally where the new sequences are inserted. Use this option instead to perform a thorough update of the phylogeny.",
                    action="store_true")
parser.add_argument('--inputRFtrees', default="",
...
parser.add_argument("--onlyNambiguities", help="Treat all ambiguities as N (total missing information).",
                    action="store_true")
parser.add_argument("--thresholdProb",
                    help="relative probability threshold used to ignore possible states with very low probabilities.",
                    type=float, default=0.00000001)
parser.add_argument("--thresholdLogLK", help="logLK difference threshold to consider a logLk close to optimal.",
                    type=float, default=200.0)
parser.add_argument("--thresholdLogLKtopology",
                    help="logLK difference threshold to consider a logLk close to optimal when looking for topology improvements.",
                    type=float, default=160.0)
parser.add_argument("--allowedFails",
                    help="Number of times one can go down the tree without inclreasing placement likelihood before the tree traversal is stopped (only applies to non-0 branch lengths).",
                    type=int, default=5)
parser.add_argument("--allowedFailsTopology",
                    help="Number of times one can crawl along the tree decreasing placement likelihood before the tree traversal is stopped during topology search (only applies to non-0 branch lengths).",
                    type=int, default=4)
...
parser.add_argument("--debugging",
                    help="Test that likelihoods are calculated and updated as expected - time consuming and only meant for small trees for debugging purposes.",
                    action="store_true")
parser.add_argument("--model",
                    help="Which substitution model should be used. Allowed models so far are JC, GTR (default) or UNREST.",
                    default="GTR")
parser.add_argument("--overwrite", help="Overwrite previous results if already present.", action="store_true")
parser.add_argument("--nonBinaryTree",
                    help="Write output tree with multifurcations - by default the tree is written as binary so to avoid problems reading the tree in other software.",
                    action="store_true")
parser.add_argument("--numTopologyImprovements",
                    help="Number of times we traverse the tree looking for topological improvements.", type=int,
                    default=1)
parser.add_argument("--thresholdTopologyPlacement",
                    help="Don't try to re-place nodes that have current appending logLK cost above this threshold.",
                    type=float, default=-0.01)
parser.add_argument("--minBLenForMidNode",
                    help="Don't try to place samples mid-branch if the considered branch is below this length in number of mutations genome-wide.",
                    type=float, default=4.1)
parser.add_argument("--updateSubstMatrixEveryThisSamples",
                    help="How many new samples to place before each update of the substitution rate matrix.", type=int,
                    default=25)
parser.add_argument("--nonStrictInitialStopRules",
                    help="If specified, then during the initial placement stage, a slower, non-strict rule for stopping the placement search is applied: the search is stopped if enough many consencutive LK worsening are observed, AND if LK is below the considered threshold.",
                    action="store_true")
parser.add_argument("--strictTopologyStopRules",
                    help="If specified, then during the topological improvement stage, a slower, non-strict rule for stopping the SPR search is applied: the search is stopped if enough many consencutive LK worsening are observed, AND if LK is below the considered threshold.",
                    action="store_true")
parser.add_argument("--thresholdDiffForUpdate",
                    help="Consider the probability of a new partial changed if the difference between old and new if above this threshold.",
                    type=float, default=0.0000001)
parser.add_argument("--thresholdFoldChangeUpdate",
                    help="Consider the probability of a new partial changed, if the fold difference between old and new if above this threshold.",
                    type=float, default=1.001)
parser.add_argument("--minBLen",
                    help="Don't attempt branch lengths below this number of mutations (genome-wide): just consider 0 length instead.",
                    type=float, default=0.2)
parser.add_argument("--maxBLen", help="Don't attempt branch lengths above this number of mutations (genome-wide).",
                    type=float, default=40.0)
parser.add_argument("--thresholdLogLKconsecutivePlacement",
                    help="logLK difference threshold to consider something as a significant decrease in log-LK when considering consecutive likelihood decreases.",
                    type=float, default=0.01)
parser.add_argument("--thresholdLogLKwholeTopologyImprovement",
                    help="logLK difference threshold to consider something as a significant decrease in log-LK when considering whole rounds of SPR moves.",
                    type=float, default=1.0)
parser.add_argument("--calculateLKfinalTree", help="Calculate the log-LK of the final infered tree.",
                    action="store_true")
parser.add_argument("--fast",
                    help="Set parameters so to run tree inference faster; this will be less accurate in cases of high complexity, for example with recombination, sequencing errors, etc. It will overrule user choices for options --thresholdLogLK , --thresholdLogLKtopology , --allowedFails , --allowedFailsTopology .",
                    action="store_true")
...
parser.add_argument("--noFastTopologyInitialSearch",
                    help="Don't run a fast short-range topology search before the extensive one.", action="store_true")
parser.add_argument("--noOptimizeBranchLengths", help="Don't run a final round of detailed branch length optimization.",
                    action="store_true")
...
if inputRFtrees != "":
	if not os.path.isfile(inputTree):
		print(
			"Input tree in newick format " + inputTree + " not found, quitting MAPLE RF distance calculation. Use option --inputTree to specify a valid input newick tree file.")
		exit()
	if not os.path.isfile(inputRFtrees):
		print(
			"Input trees in newick format " + inputRFtrees + " not found, quitting MAPLE RF distance calculation. Use option --inputRFtrees to specify valid file with input newick trees.")
		exit()
	...
	if not os.path.isdir(outFolder):
		print(
			"Path to output file " + outFolder + " does not exist, quitting MAPLE RF calculation. Use option --output to specify a valid output file path and output file name.")
		exit()
	if os.path.isfile(outputFile + "_RFdistances.txt") and (not overwrite):
		print(
			"File " + outputFile + "_RFdistances.txt already exists, quitting MAPLE RF calculation. Use option --overwrite if you want to overwirte previous inference.")
		exit()
	...
	for tree in otherTrees:
		numDiffs, normalisedRF, leafCount, foundBranches, missedBranches, notFoundBranches = RobinsonFouldsWithDay1985(
			tree, leafNameDict, nodeTable, leafCount, numBranches, rooted=False)
		file.write(
			str(numDiffs) + "\t" + str(normalisedRF) + "\t" + str(leafCount) + "\t" + str(foundBranches) + "\t" + str(
				missedBranches) + "\t" + str(notFoundBranches) + "\n")
...
if os.path.isfile(outputFile + "_tree.tree") and (not overwrite):
	print(
		"File " + outputFile + "_tree.tree already exists, quitting MAPLE tree inference. Use option --overwrite if you want to overwirte previous inference.")
	exit()
if not os.path.isfile(inputFile):
	print(
		"Input file in Maple format " + inputFile + " not found, quitting MAPLE tree inference. Use option --input to specify a valid input file.")
	exit()
if refFile != "" and (not os.path.isfile(refFile)):
	print(
		"Input reference fasta file " + refFile + " not found, quitting MAPLE tree inference. Use option --reference to specify a valid input reference file.")
	exit()
lineSplit = outputFile.split("/")
lineSplit[-1] = ""
outFolder = "/".join(lineSplit)
if not os.path.isdir(outFolder):
	print(
		"Path to output file " + outFolder + " does not exist, quitting MAPLE tree inference. Use option --output to specify a valid output file path and output file name.")
	exit()
if inputTree != "":
	if not os.path.isfile(inputTree):
		print(
			"Input tree in newick format " + inputTree + " not found, quitting MAPLE. Use option --inputTree to specify a valid input newick tree file.")
		exit()
...
ambiguities = {"y": [0.0, 0.5, 0.0, 0.5], "r": [0.5, 0.0, 0.5, 0.0], "w": [0.5, 0.0, 0.0, 0.5],
               "s": [0.0, 0.5, 0.5, 0.0], "k": [0.0, 0.0, 0.5, 0.5], "m": [0.5, 0.5, 0.0, 0.0],
               "d": [1.0 / 3, 0.0, 1.0 / 3, 1.0 / 3], "v": [1.0 / 3, 1.0 / 3, 1.0 / 3, 0.0],
               "h": [1.0 / 3, 1.0 / 3, 0.0, 1.0 / 3], "b": [0.0, 1.0 / 3, 1.0 / 3, 1.0 / 3]}
...
def updateSubMatrix(pseudoMutCounts, model, oldMutMatrix):  # ,mutMatrixBLen,mutMatrixBLenAdjusted
	...
	if model == "UNREST":
		...
		exit()
	totRate = -(rootFreqs[0] * mutMatrix[0][0] + rootFreqs[1] * mutMatrix[1][1] + rootFreqs[2] * mutMatrix[2][2] +
	            rootFreqs[3] * mutMatrix[3][3])
	for i in range4:
		for j in range4:
...
def mergeVectors(probVect1, bLen1, probVect2, bLen2, mutMatrix, returnLK=False, useRateVariation=False,
                 mutMatrices=None):
	indexEntry1, indexEntry2, pos = 0, 0, 0
	probVect = []
	cumulPartLk = 0.0
	...
	while True:
		if entry1[0] == 5:
			...
			if entry2[0] == entry1[0] and entry2[0] < 5:  # entry1 and entry2 are two identical nucleotides
				...
				pos = end
			elif (not totLen1) and (not totLen2) and entry1[0] < 5 and entry2[
				0] < 5:  # 0 distance between different nucleotides: merge is not possible
				if returnLK:
...
def reCalculateAllGenomeLists(root, mutMatrix, checkExistingAreCorrect=False, countNodes=False, countPseudoCounts=False,
                              pseudoMutCounts=None, data=None, useRateVariation=False, mutMatrices=None):
			...
			if node.children:
				...
				if data != None:
					if node.name in data:
						...
						print(
							"Error: sample name " + node.name + " not found in the input sequence data - all samples in the input tree need to have a corresponding sequence entry.")
						exit()
				if countNodes:
					numNodes[0] += 1
			...
			if lastNode == node.children[0]:
				...
				if newLower == None:
					if not node.children[0].dist:
						nodeList = []
						updateBLen(nodeList, node, mutMatrix, useRateVariation=useRateVariation,
						...
						nodeList = []
						updateBLen(nodeList, node.children[1], mutMatrix, useRateVariation=useRateVariation,
						           mutMatrices=mutMatrices)
						updatePartials(nodeList, mutMatrix, useRateVariation=useRateVariation, mutMatrices=mutMatrices)
					else:
						print(
							"Strange, distances>0 but inconsistent lower genome list creation in reCalculateAllGenomeLists()")
						exit()
				else:
	...
	if node.children:
		...
		if checkExistingAreCorrect:
			if areVectorsDifferentDebugging(newVect, node.probVectUpLeft):
				print(
					"new probVectUpLeft at root is different from the old one, and it shouldn't be; distance of child node " + str(
						node.children[0].dist))
				print(node.dist)
				print(node.children)
			...
			if direction == 0:
				...
				if node.children:
					newUpRight = mergeVectorsUpDown(vectUp, node.dist, node.children[1].probVect, node.children[1].dist,
					                                mutMatrix, useRateVariation=useRateVariation,
					                                mutMatrices=mutMatrices)
					if newUpRight == None:
						if (not node.children[1].dist):
							...
							updateBLen(nodeList, node.children[1], mutMatrix, useRateVariation=useRateVariation,
							           mutMatrices=mutMatrices)
							updatePartials(nodeList, mutMatrix, useRateVariation=useRateVariation,
							               mutMatrices=mutMatrices)
						elif (not node.dist):
							nodeList = []
							updateBLen(nodeList, node, mutMatrix, useRateVariation=useRateVariation,
							...
							print(
								"Strange, distances>0 but inconsistent upRight genome list creation in reCalculateAllGenomeLists()")
							exit()
					else:
						if checkExistingAreCorrect:
					...
					if newUpLeft == None:
						if (not node.children[0].dist):
							nodeList = []
							updateBLen(nodeList, node.children[0], mutMatrix, useRateVariation=useRateVariation,
							           mutMatrices=mutMatrices)
							updatePartials(nodeList, mutMatrix, useRateVariation=useRateVariation,
							               mutMatrices=mutMatrices)
						elif (not node.dist):
							...
							print(
								"Strange, distances>0 but inconsistent upLeft genome list creation in reCalculateAllGenomeLists()")
							exit()
					else:
						if checkExistingAreCorrect:
...
if model == "JC":
	mutMatrix = [[-1.0, 1.0 / 3, 1.0 / 3, 1.0 / 3], [1.0 / 3, -1.0, 1.0 / 3, 1.0 / 3],
		...
		if entry2[0] == 5:  # case N
			...
			if entry2[0] == 4:
				...
				if len(entry1) == 2:
					...
					i1 = refIndeces[pos]
					i2 = entry2[0]
					totalFactor *= ((rootFreqs[i1] * mutMatrix[i1][i2] * (bLen + entry1[3]) * (
								1.0 + nonMutRates[i1] * entry1[2]) + rootFreqs[i2] * mutMatrix[i2][i1] * entry1[2] * (
								                 1.0 + nonMutRates[i2] * (bLen + entry1[3]))) / rootFreqs[i1])
				pos += 1
		...
		# entry1 is of type "O"
			...
			if entry2[0] == 6:
				...
				if entry2[0] == 4:
					...
					i2 = entry2[0]
				totalFactor *= (entry1[-1][i2] + bLen13 * (
							entry1[-1][0] * mutMatrix[0][i2] + entry1[-1][1] * mutMatrix[1][i2] + entry1[-1][2] *
							mutMatrix[2][i2] + entry1[-1][3] * mutMatrix[3][i2]))
			pos += 1
			...
			if entry2[0] == i1:
				...
				# entry2 is a nucleotide type (like entry1)
					...
					if len(entry1) == 2:
						...
						totalFactor *= ((rootFreqs[i1] * mutMatrix[i1][i2] * (bLen + entry1[3]) * (
								1.0 + nonMutRates[i1] * entry1[2]) + rootFreqs[i2] * mutMatrix[i2][i1] * entry1[
							                 2] * (1.0 + nonMutRates[i2] * (bLen + entry1[3]))) / rootFreqs[i1])
			pos += 1
...
def findBestParentTopology(node, child, bestLKdiff, removedBLen, mutMatrix,
                           strictTopologyStopRules=strictTopologyStopRules, allowedFailsTopology=allowedFailsTopology,
                           thresholdLogLKtopology=thresholdLogLKtopology, useRateVariation=False, mutMatrices=None):
	bestNode = node
	bestNodes = []
	nodesToVisit = []
	...
	if node.up != None:
		...
		nodesToVisit.append((node.up, childUp, node.children[1 - child].probVect,
		                     node.children[1 - child].dist + node.dist, True, bestLKdiff, 0))
		nodesToVisit.append(
			(node.children[1 - child], 0, vectUpUp, node.children[1 - child].dist + node.dist, True, bestLKdiff, 0))
	else:
		# case node is root
		if node.children[
			1 - child].children:  # case there is only one sample outside of the subtree doesn't need to be considered
			child1 = node.children[1 - child].children[0]
			child2 = node.children[1 - child].children[1]
			vectUp1 = rootVector(child2.probVect, child2.dist, mutMatrix, useRateVariation=useRateVariation,
			                     mutMatrices=mutMatrices)
			nodesToVisit.append((child1, 0, vectUp1, child1.dist, True, bestLKdiff, 0))
			vectUp2 = rootVector(child1.probVect, child1.dist, mutMatrix, useRateVariation=useRateVariation,
			                     mutMatrices=mutMatrices)
	...
	while nodesToVisit:
		...
		if direction == 0:
			...
			if t1.dist and (not (t1.up == node or t1.up == None)):
				...
				if midProb < (
						lastLK - thresholdLogLKconsecutivePlacement):  # placement at current node is considered failed if placement likelihood is not improved by a certain margin compared to best placement so far for the nodes above it.
					failedPasses += 1
			else:
				midProb = lastLK
			...
			if strictTopologyStopRules:
				if failedPasses <= allowedFailsTopology and midProb > (
						bestLKdiff - thresholdLogLKtopology) and t1.children:
			...
			if keepTraversing:
				...
				if t1.up != None:  # case the node is not the root
					...
					if vectUp == None:
						...
						nodesToVisit.append(
							(otherChild, 0, vectUp, otherChild.dist, needsUpdating, midProb, failedPasses))
				...
				# now consider case of root node
					...
					if needsUpdating:
						vectUp = rootVector(passedPartials, distance, mutMatrix, useRateVariation=useRateVariation,
						                    mutMatrices=mutMatrices)
					else:
						if direction == 1:
							vectUp = t1.probVectUpLeft
	...
	while nodesToVisit:
		...
		if t1.dist and t1.up != None:  # try first placing as a descendant of the mid-branch point of the branch above the current node.
			...
			if LKdiff >= bestLKdiff:
				...
				bestNodes.append((t1, LKdiff))
			if LKdiff < (
					parentLK - thresholdLogLKconsecutivePlacement):  # placement at current node is considered failed if placement likelihood is not improved by a certain margin compared to best placement so far for the nodes above it.
				failedPasses += 1
		else:
			LKdiff = parentLK
			...
			for i in range4:
				...
				if diffVal:
					if (not entry1[-1][i]) or (not entry2[-1][i]):
						return True
					if diffVal > thresholdDiffForUpdate or (diffVal > thresholdProb and (
							(diffVal / entry1[-1][i] > thresholdFoldChangeUpdate) or (
							diffVal / entry2[-1][i] > thresholdFoldChangeUpdate))):
						return True
		# update pos, end, etc
		pos = min(entry1[1], entry2[1])
		if pos == lRef:
			...
			for i in range4:
				...
				if diffVal:
					if (not entry1[-1][i]) or (not entry2[-1][i]):
						...
						return True
					if diffVal > 0.01 or (diffVal > threshold and (
							(diffVal / entry1[-1][i] > thresholdFoldChangeUpdate) or (
							diffVal / entry2[-1][i] > thresholdFoldChangeUpdate))):
...
def updateBLen(nodeList, node, mutMatrix, useRateVariation=False, mutMatrices=None):
	...
	if cNode == node.children[0]:
		...
		cNum = 1
	bestLength = estimateBranchLengthWithDerivative(vectUp, cNode.probVect, mutMatrix,
	                                                useRateVariation=useRateVariation, mutMatrices=mutMatrices)
...
def updatePartials(nodeList, mutMatrix, useRateVariation=False, mutMatrices=None):
	while nodeList:
		...
		if direction == 2:
			...
			not updatedBLen):  # at valid internal node, update upLeft and upRight, and if necessary add children to nodeList.
				...
				if not updatedBLen:
					...
					if newUpLeft == None:
						if (not node.dist) and (not dist0):
							...
							print(
								"Strange: None vector from non-zero distances in updatePartials() from parent direction, child0.")
							exit()
						updatedBLen = True
				if not updatedBLen:
			...
			if newVect == None:
				if (not childDist) and (not otherChildDist):
					updateBLen(nodeList, node.children[childNum], mutMatrix, useRateVariation=useRateVariation,
					           mutMatrices=mutMatrices)
					updatedBLen = True
				else:
			...
			if not updatedBLen:
				...
				if node.up != None:
					...
					newUpVect = rootVector(probVectDown, childDist, mutMatrix, useRateVariation=useRateVariation,
					                       mutMatrices=mutMatrices)
				if newUpVect == None:
					if (not node.dist) and (not childDist):
						...
						print(
							"Strange: None vector from non-zero distances in updatePartials() from child direction, newUpVect.")
						exit()
				else:
...
# TODO update to use analytical branch length optimization - done
def placeSampleOnTree(node, newPartials, sample, newChildLK, bestUpLength, bestDownLength, bestAppendingLength,
                      mutMatrix, pseudoMutCounts):
	tryNewRoot = False
	if node.up == None:
		tryNewRoot = True
	...
	if tryNewRoot:
		...
		if secondBranchLengthOptimizationRound:  # if wanted, do a second round of branch length optimization
			...
			rootUpRight = rootVector(newPartials, bestRightLength, mutMatrix)
			bestLeftLength = estimateBranchLengthWithDerivative(rootUpRight, node.probVect, mutMatrix)
		probVectRoot, probRoot = mergeVectors(node.probVect, bestLeftLength, newPartials, bestRightLength, mutMatrix,
		                                      returnLK=True)
		...
		if bestRightLength:
			newNode.probVectTotUp = mergeVectorsUpDown(newRoot.probVectUpLeft, bestRightLength / 2, newPartials,
			                                           bestRightLength / 2, mutMatrix)
		if verbose:
	...
	if bestUpLength:
		newInternalNode.probVectTotUp = mergeVectorsUpDown(vectUp, bestUpLength / 2, newInternalNode.probVect,
		                                                   bestUpLength / 2, mutMatrix)
	if bestAppendingLength:
		newNode.probVectTotUp = mergeVectorsUpDown(newInternalNode.probVectUpLeft, bestAppendingLength / 2, newPartials,
		                                           bestAppendingLength / 2, mutMatrix)
					...
					if len(entry1) == 4:
						...
						if contribLength:
							totalFactor *= ((rootFreqs[i1] * mutMatrix[i1][i2] * contribLength * (
										1.0 + mutMatrix[i1][i1] * entry1[2]) + rootFreqs[i2] * mutMatrix[i2][i1] *
							                 entry1[2] * (1.0 + mutMatrix[i2][i2] * contribLength)) / rootFreqs[i1])
						else:
			...
			# entry1 is of type "O"
				...
				if entry2[0] == 6:
					if contribLength:
						tot = 0.0
						for j in range4:
							tot += entry1[-1][j] * (entry2[-1][j] + contribLength * (
										mutMatrix[j][0] * entry2[-1][0] + mutMatrix[j][1] * entry2[-1][1] +
										mutMatrix[j][2] * entry2[-1][2] + mutMatrix[j][3] * entry2[-1][3]))
						totalFactor *= tot
					else:
						tot = 0.0
						for j in range4:
					...
					if entry2[0] == 4:
						...
						i2 = entry2[0]
					if contribLength:
						totalFactor *= (entry1[-1][i2] + contribLength * (
								entry1[-1][0] * mutMatrix[0][i2] + entry1[-1][1] * mutMatrix[1][i2] + entry1[-1][
							2] * mutMatrix[2][i2] + entry1[-1][3] * mutMatrix[3][i2]))
					else:
		...
		if entry2[0] == 5:  # case entry1 is N
			...
			if entry1[0] == 4:  # case entry1 is R
				...
				# entry1 is reference and entry2 is of type "O"
					...
					if len(entry1) == 4:
						...
						i2 = entry2[0]
						if contribLength:
							coeff0 = rootFreqs[i1] * mutMatrix[i1][i2] * contribLength + rootFreqs[i2] * mutMatrix[i2][
								i1] * entry1[2]
			...
			# entry1 is of type "O"
				...
				if useRateVariation:
					mutMatrix = mutMatrices[pos]
				if entry2[0] == 6:
					coeff0 = entry1[-1][0] * entry2[-1][0] + entry1[-1][1] * entry2[-1][1] + entry1[-1][2] * entry2[-1][
						2] + entry1[-1][3] * entry2[-1][3]
					coeff1 = 0.0
					for i in range4:
						for j in range4:
				...
				if entry2[0] == entry1[0]:
					...
					if entry2[0] < 5:  # entry2 is a nucleotide
						...
						if len(entry1) == 4:
							if contribLength:
								coeff0 = rootFreqs[i1] * mutMatrix[i1][i2] * contribLength + rootFreqs[i2] * \
								         mutMatrix[i2][i1] * entry1[2]
								coeff1 = rootFreqs[i1] * mutMatrix[i1][i2]
							else:
								coeff0 = rootFreqs[i2] * mutMatrix[i2][i1] * entry1[2]
								coeff1 = rootFreqs[i1] * mutMatrix[i1][i2]
...
def traverseTreeToOptimizeBranchLengths(root, mutMatrix, testing=False, useRateVariation=False, mutMatrices=None):
	...
	while nodesToTraverse:
		...
		if node == node.up.children[0]:
			...
			child = 1
		if node.dirty:
			bestLength = estimateBranchLengthWithDerivative(upVect, node.probVect, mutMatrix,
			                                                useRateVariation=useRateVariation, mutMatrices=mutMatrices)
			if bestLength or node.dist:
				if testing:
					currentCost = appendProbNode(upVect, node.probVect, node.dist, mutMatrix,
					                             useRateVariation=useRateVariation, mutMatrices=mutMatrices)
					newCost = appendProbNode(upVect, node.probVect, bestLength, mutMatrix,
					                         useRateVariation=useRateVariation, mutMatrices=mutMatrices)
				...
				if (not bestLength) or (
				not node.dist) or node.dist / bestLength > 1.01 or node.dist / bestLength < 0.99:
...
# TODO update to use analytical branch length optimization - done
def placeSubtreeOnTree(node, newPartials, appendedNode, newChildLK, bestBranchLengths, mutMatrix,
                       useRateVariation=False, mutMatrices=None):
	...
	if tryNewRoot:
		node = root
		probOldRoot = findProbRoot(node.probVect)
		rootUpLeft = rootVector(node.probVect, bestAppendingLength / 2, mutMatrix, useRateVariation=useRateVariation,
		                        mutMatrices=mutMatrices)
		bestRightLength = estimateBranchLengthWithDerivative(rootUpLeft, newPartials, mutMatrix,
		                                                     useRateVariation=useRateVariation, mutMatrices=mutMatrices)
		rootUpRight = rootVector(newPartials, bestRightLength, mutMatrix, useRateVariation=useRateVariation,
		                         mutMatrices=mutMatrices)
		bestLeftLength = estimateBranchLengthWithDerivative(rootUpRight, node.probVect, mutMatrix,
		                                                    useRateVariation=useRateVariation, mutMatrices=mutMatrices)
		secondBranchLengthOptimizationRound = True
		if secondBranchLengthOptimizationRound:  # if wanted, do a second round of branch length optimization
			rootUpLeft = rootVector(node.probVect, bestLeftLength, mutMatrix, useRateVariation=useRateVariation,
			                        mutMatrices=mutMatrices)
			bestRightLength = estimateBranchLengthWithDerivative(rootUpLeft, newPartials, mutMatrix,
		...
		newRoot.probVect = probVectRoot
		newRoot.probVectUpRight = rootUpRight
		newRoot.probVectUpLeft = rootVector(node.probVect, bestLeftLength, mutMatrix, useRateVariation=useRateVariation,
		                                    mutMatrices=mutMatrices)
		node.up = newRoot
		node.dist = bestLeftLength
		newRoot.add_child(node)
...
def cutAndPasteNode(node, bestNode, bestBranchLengths, bestLK, mutMatrix, useRateVariation=False, mutMatrices=None):
	...
	# updatePartialsFromTop(sibling.children[1],sibling.probVectUpLeft,mutMatrix)
		...
		if verbose:
			if sibling.dist:
				print(
					"cutAndPasteNode(), sibling node is not root. New overall likelihoods at previously sibling node: ")
	...
	newRoot = placeSubtreeOnTree(bestNode, node.probVect, node, bestLK, bestBranchLengths, mutMatrix,
	                             useRateVariation=useRateVariation, mutMatrices=mutMatrices)
	# if debugging:
	# 	root=node
	# 	while root.up!=None:
...
# TODO update to use analytical branch length optimization - done
def traverseTreeForTopologyUpdate(node, mutMatrix, strictTopologyStopRules=strictTopologyStopRules,
                                  allowedFailsTopology=allowedFailsTopology,
                                  thresholdLogLKtopology=thresholdLogLKtopology,
                                  thresholdTopologyPlacement=thresholdTopologyPlacement, useRateVariation=False,
                                  mutMatrices=None):
	...
	if node.up != None:
		...
		originalLK = appendProbNode(vectUp, node.probVect, bestCurrenBLen, mutMatrix, useRateVariation=useRateVariation,
		                            mutMatrices=mutMatrices)
		bestCurrentLK = originalLK
		if bestCurrentLK < thresholdTopologyPlacement:
			bestCurrenBLen = estimateBranchLengthWithDerivative(vectUp, node.probVect, mutMatrix,
			                                                    useRateVariation=useRateVariation,
			                                                    mutMatrices=mutMatrices)
			if bestCurrenBLen or node.dist:
				if (not bestCurrenBLen) or (
				not node.dist) or node.dist / bestCurrenBLen > 1.01 or node.dist / bestCurrenBLen < 0.99:
					bLenChanged = True
		...
		if bestCurrentLK < thresholdTopologyPlacement:
			...
			if debugging:
				if bLenChanged:
					print(
						"Better branch length found at node " + str(node) + ", from " + str(node.dist) + " (LK " + str(
							originalLK) + ") to " + str(bestCurrenBLen) + " (LK " + str(bestCurrentLK) + "). Children")
					print(node.children)
			...
			bestNodeSoFar, bestLKdiff, bestBranchLengths = findBestParentTopology(parentNode, child, bestCurrentLK,
			                                                                      bestCurrenBLen, mutMatrix,
			                                                                      strictTopologyStopRules=strictTopologyStopRules,
			                                                                      allowedFailsTopology=allowedFailsTopology,
			                                                                      thresholdLogLKtopology=thresholdLogLKtopology,
			                                                                      useRateVariation=useRateVariation,
			                                                                      mutMatrices=mutMatrices)
			...
			if bestLKdiff + thresholdTopologyPlacement > bestCurrentLK:
				...
				if topologyUpdated:
					totalImprovement = (bestLKdiff - originalLK) #I changed this to originalLK!
					if verbose:
						print("\n\n In traverseTreeForTopologyUpdate() found SPR move with improvement " + str(
							totalImprovement))
					if debugging:
					...
					newRoot = cutAndPasteNode(node, bestNodeSoFar, bestBranchLengths, bestLKdiff, mutMatrix,
					                          useRateVariation=useRateVariation, mutMatrices=mutMatrices)
					bLenChanged = False
		if (not topologyUpdated) and bLenChanged:
			if debugging:
				print("Changing branch length (pos 3) from " + str(node.dist) + " to " + str(
					bestCurrenBLen) + " at node " + str(node) + " with children ")
				print(node.children)
...
def startTopologyUpdates(node, mutMatrix, checkEachSPR=False, strictTopologyStopRules=strictTopologyStopRules,
                         allowedFailsTopology=allowedFailsTopology, thresholdLogLKtopology=thresholdLogLKtopology,
                         thresholdTopologyPlacement=thresholdTopologyPlacement, useRateVariation=False,
                         mutMatrices=None):
	nodesToVisit = [node]
	totalImprovement = 0.0
	...
	while nodesToVisit:
		...
		if newNode.dirty:
			...
			newRoot2, improvement = traverseTreeForTopologyUpdate(newNode, mutMatrix,
			                                                      ...
			                                                      allowedFailsTopology=allowedFailsTopology,
			                                                      thresholdLogLKtopology=thresholdLogLKtopology,
			                                                      thresholdTopologyPlacement=thresholdTopologyPlacement,
			                                                      useRateVariation=useRateVariation,
			                                                      mutMatrices=mutMatrices)
			if checkEachSPR:
				...
				newTreeLK = calculateTreeLikelihood(root, mutMatrix, useRateVariation=useRateVariation,
				                                    mutMatrices=mutMatrices)
				reCalculateAllGenomeLists(root, mutMatrix, checkExistingAreCorrect=True,
				                          useRateVariation=useRateVariation, mutMatrices=mutMatrices)
				# print("Post-SPR tree likelihood: "+str(newTreeLK))
				print("In startTopologyUpdates, LK score of improvement " + str(newTreeLK) + " - " + str(
					oldTreeLK) + " = " + str(newTreeLK - oldTreeLK) + ", was supposed to be " + str(improvement))
				if newTreeLK - oldTreeLK < improvement - 1.0:
					print("In startTopologyUpdates, LK score of improvement " + str(newTreeLK) + " - " + str(
						oldTreeLK) + " = " + str(newTreeLK - oldTreeLK) + " is less than what is supposd to be " + str(
						improvement))
					exit()
		...
		# these are anyway the same for all trees and substitution models!
			...
			if lastNode == node.children[0]:
				...
				newLower, Lkcontribution = mergeVectors(node.children[0].probVect, node.children[0].dist,
				                                        ...
				                                        returnLK=True, useRateVariation=useRateVariation,
				                                        mutMatrices=mutMatrices)
				totalLK += Lkcontribution
				if newLower == None:
					print(
						"Strange, inconsistent lower genome list creation in calculateTreeLikelihood(); old list, and children lists")
					print(node.probVect)
					print(node.children[0].probVect)
					print(node.children[1].probVect)
					exit()
				elif numTopologyImprovements and checkCorrectness and areVectorsDifferentDebugging(node.probVect,
				                                                                                   newLower):
					print(
						"Strange, while calculating tree likelihood encountered non-updated lower likelihood genome list at node " + str(
							node) + " with children ")
					print(node.children)
					print("new list")
					print(newLower)
...
def expectationMaximizationCalculationRates(root, useRateVariation=False):
									...
									if len(entry1) == 4:
										...
										for i in range4:
											if i1 == i:
												...
												prob = rootFreqs[i] * mutMatrix[i][i1] * entry1[2] * (
															1.0 + nonMutRates[i] * contribLength) * entry2[-1][i]
												normalization += prob
										for i in range4:
											if i1 == i:
												prob = rootFreqs[i] * (1.0 + nonMutRates[i] * entry1[2])
												for j in range4:
													if j == i:
														tot3 = prob * (1.0 + nonMutRates[i] * contribLength) * \
														       entry2[-1][j]
														waitingTimes[i] += contribLength * tot3 / normalization
														if useRateVariation:
															waitingTimesSites[pos][
																i] += contribLength * tot3 / normalization
													else:
														tot3 = prob * mutMatrix[i][j] * contribLength * entry2[-1][
															j] / normalization
								...
								if not totLen2:
									...
									if len(entry1) < 4:
										...
										prob1 = rootFreqs[i1] * mutMatrix[i1][i2] * contribLength * (
													1.0 + nonMutRates[i1] * entry1[2])
										prob2 = rootFreqs[i2] * mutMatrix[i2][i1] * entry1[2] * (
													1.0 + nonMutRates[i2] * contribLength)
									...
									for i in range4:
										for j in range4:
											if i == j:
												...
												normalization += entry1[-1][i] * mutMatrix[i][j] * totLen1 * entry2[-1][
													j]
									for i in range4:
										for j in range4:
											if i == j:
												...
												prob = entry1[-1][i] * mutMatrix[i][j] * totLen1 * entry2[-1][
													j] / normalization
												waitingTimes[i] += (totLen1 / 2) * prob
										...
										if len(entry1) == 4:
											...
											for i in range4:
												if i1 == i:
													...
													if useRateVariation:
														waitingTimesSites[pos][
															i] += contribLength * prob / normalization
										else:
											if useRateVariation:
												waitingTimesSites[pos][refIndeces[pos]] -= totLen1
											for i in range4:
	...
	if model == "UNREST":
		...
		exit()
	totRate = -(rootFreqs[0] * counts[0][0] + rootFreqs[1] * counts[1][1] + rootFreqs[2] * counts[2][2] + rootFreqs[3] *
	            counts[3][3])
	if totRate:
		for i in range4:
			for j in range4:
...
while distances:
	...
	if bestScore < 0.5:
		start = time()
		newRoot = placeSampleOnTree(bestNode, newPartials, sample, bestScore, bestBranchLengths[0],
		                            bestBranchLengths[1], bestBranchLengths[2], mutMatrix, pseudoMutCounts)
		if newRoot != None:
			t1 = newRoot
	...
	if model != "JC" or rateVariation:
		...
		if rateVariation:
			...
			for i in range(lRef):
				cumulativeRate[i + 1] = cumulativeRate[i] + nonMutRates[refIndeces[i]]
			mutMatrices = None
		reCalculateAllGenomeLists(t1, mutMatrix, countNodes=False, useRateVariation=rateVariation,
		                          mutMatrices=mutMatrices)
...
if fastTopologyInitialSearch and (inputTree == "" or largeUpdate):
	...
	newRoot, improvement = startTopologyUpdates(t1, mutMatrix, checkEachSPR=debugging,
	                                            strictTopologyStopRules=strictTopologyStopRulesInitial,
	                                            allowedFailsTopology=allowedFailsTopologyInitial,
	                                            thresholdLogLKtopology=thresholdLogLKtopologyInitial,
	                                            thresholdTopologyPlacement=thresholdTopologyPlacementInitial,
	                                            useRateVariation=rateVariation, mutMatrices=mutMatrices)
	if newRoot != None:
		t1 = newRoot
	timeForUpdatingTopology = (time() - start)
	print("Time for initial traversal of the tree for only short range topology changes or branch length: " + str(
		timeForUpdatingTopology))
...
for i in range(numTopologyImprovements):
	...
	if inputTree == "" or largeUpdate:
		setAllDirty(t1)
	newRoot, improvement = startTopologyUpdates(t1, mutMatrix, checkEachSPR=debugging, useRateVariation=rateVariation,
	                                            mutMatrices=mutMatrices)
	if newRoot != None:
		t1 = newRoot
	timeForUpdatingTopology = (time() - start)
	print(
		"Time for this traversal of the tree to update the topology or branch length: " + str(timeForUpdatingTopology))
	timeTopology += timeForUpdatingTopology
	print("LK improvement apparently brought: " + str(improvement))
...
if optimizeBranchLengths:
	...
	improvement = traverseTreeToOptimizeBranchLengths(t1, mutMatrix, testing=testingBLen,
	                                                  useRateVariation=rateVariation, mutMatrices=mutMatrices)
	if testingBLen:
	...
	while subRound < 20:
		if improvement < thresholdLogLKwholeTopologyImprovement:
			break
		subRound += 1
		improvement = traverseTreeToOptimizeBranchLengths(t1, mutMatrix, useRateVariation=rateVariation,
		                                                  mutMatrices=mutMatrices)
		if testingBLen:
