...
def prepareTreeComparison(t1,rooted=False,minimumBLen=0.000006, addRootRFL = False):
    ...
    nodeTable=[]
    #myrthe: hash table with branch lengths
    ...
    while node!=t2.up:
        if movingFrom==0: #0 means reaching node from parent, 1 means coming back from a child
            if len(node.children)==0:
                ...
                # I have not added branch lengths of leaf nodes to sumBranchLengths, so no need for: RFL=- trueDist
            else:
                node.exploredChildren=0
                node.maxSoFar=float("-inf")
                node.minSoFar=float("inf")
            ...
            if node.exploredChildren==nChildren:
                nextNode=node.up
                movingFrom=1
                lastL=node.minSoFar #Question what are minsofar and maxsofar?
                lastR=node.maxSoFar
                lastDesc=node.nDescendants
                if node!=t2:
                    if node.dist>minimumBLen:
...
def traverseTreeToOptimizeBranchLengths(root,mutMatrix,testing=False,useRateVariation=False,mutMatrices=None):
    ...
    while nodesToTraverse:
        # if 't1' and 'test_if_different' and 't0_1' in globals(): #remove
        ...
        if node.dirty:
            ...
            if 'best_lengths_t0' in globals(): #remove
...
def addErrorTerminalNode(node, errorRateOneThird):
    ...
    """
    # simply change ambiguities list --> faster method.
    # print(node.name)
    # print(node.probVect)
        ...
        if entry1[0] == 5: # Case τ1 = N
            ...
            if entry2[0] == entry1[0] and entry2[0] < 5:  # entry1 and entry2 are two identical nucleotides #Case τ1 = τ2 ϵ {A, C, T, G, R}
                ...
                if entry2[0] == 6:  # entry1 is a nucleotide and entry2 is "O"
                    ...
                    if totLen2 or (flag2 and errorRate): #Question: In contrast to mergeVectorsUpDown, here, in case totLen2==0,
...
# activateErrorFunctions(activate=False)
# startTopologyUpdates(t0_1,mutMatrix,checkEachSPR=debugging,strictTopologyStopRules=strictTopologyStopRulesInitial,allowedFailsTopology=allowedFailsTopologyInitial,thresholdLogLKtopology=thresholdLogLKtopologyInitial,thresholdTopologyPlacement=thresholdTopologyPlacementInitial,useRateVariation=rateVariation,mutMatrices=mutMatrices)
#
# #startTopologyUpdates(t0_1,mutMatrix,checkEachSPR=debugging,useRateVariation=rateVariation,mutMatrices=mutMatrices)
# activateErrorFunctions(activate=True)
# #startTopologyUpdates(t1,mutMatrix,checkEachSPR=debugging,useRateVariation=rateVariation,mutMatrices=mutMatrices)
# startTopologyUpdates(t1,mutMatrix,checkEachSPR=debugging,strictTopologyStopRules=strictTopologyStopRulesInitial,allowedFailsTopology=allowedFailsTopologyInitial,thresholdLogLKtopology=thresholdLogLKtopologyInitial,thresholdTopologyPlacement=thresholdTopologyPlacementInitial,useRateVariation=rateVariation,mutMatrices=mutMatrices)
#
# traverseTwoTopologies(t1,t0_1)
...
if benchmarkingFile:
    ...
    if not os.path.exists(benchmarkingFile):
        ...
        file.write( "inputFile\t" + "repeat\t" + "errorRateInEstimation\t" + "errorRateInSimulation\t" +   "lRef\t"+ "leaves\t" + "||\t" +
        ...
        #     0) +calculateLKfinalTree "\t" + str(totBlenTrue) + "\n")
        file.close()
    try:
    ...
    file.write(str(inputFile) + "\t" + repeat + "\t" + str(errorRate) + "\t" + errorRateSimulated +  "\t"  + str(lRef) + "\t" + str(leafCount) + "\t||\t" +
